<template>
  <div class="perforfinish">
    ...
    <div class="finish">
      ...
      <p>恭喜您，已成功重置密码！</p>
      <p id="turnback">{{count}}秒后将自动返回登录页</p>
      <button @click="perforfinishback()">返回登录页</button>
      <!--完成页面end  -->
    </div>
  </div>
</template>
...
.perforfinish > .finish img {
  width: 7rem;
  height: 7rem;
  margin-bottom: 1rem;
}
.perforfinish .finish p {
  margin: 0.5rem;
}
.perforfinish > .finish p:nth-child(2) {
  font-size: 1.5rem;
}
.perforfinish .finish p:nth-child(3) {
  letter-spacing: 0.2em;
  font-size: 1.25rem;
}
.perforfinish .finish button {
  background-color: #007acc;
  border: none;
  outline: none;
  color: white;
